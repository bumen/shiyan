 ...
 * 抽象工厂
   ...
   + 角色
     ...
     - 具体产品类
   + 区别
     - 与工厂方法区别：工厂类可以有产出多个产品，工厂方法只产出一种产品
 * 单例
   + 枚举实现
   + 饿汉
   + 懒汉
 ...
 * 原型模式
   ...
   + 角色 
      - 客户角色
      - 抽象原型角色：给出原型接口如：clone()
      - 具体原型角色：被复制对象
   
#### 结构模式 8种
 * 意思
   + 把几个类之间的几种关系确定，形成一个固定结构。通过这个结构来完成功能，这个结构不会涉及到多个功能动态切换，
   > 目的：实现一个功能的灵活性
      - 创建关系
      - 调用关系
      - 继承关系
      - 复合关系
   + 对象结构中：结构是动态的，每个对象都是要完成相关功能
 * 类结构与对象结构两种
 * 适配器
   + 类适配器与对象适配器
   + 将一个接口适配成另一个接口
 ...
 * 代理
   ...
   + 智能代理
   + 角色
     - 抽象主题角色：真实主题和代理主题的共同接口，这样一来任何使用真实主题的地方都可以使用代理主题
     - 代理主题：含有对真实主题的引用
     - 真实主题
 * 合成
   + 有向树：由上向下，由下向上，双向
 ...
 * 门面
   ...
   + 角色
     - 门面角色：
     - 子系统角色：每个子系统都不是一个单独的类，而是一个类的集合。每一个子系统都可以被客户端直接调用或被门面角色调用，
     子系统并不知道门面的存在，门面仅仅是另外一个客户端而已
 ...
 * 桥梁
   ...
   + 目的：为同一个抽象化角色提供不同的实现
   + 真正桥梁模式：涉及到两个类体系结构（抽象体系+实现体系）
   + 角色
     - 抽象化角色：保存一个对实现化对象的引用
     - 修正抽象化角色：扩展抽象化角色，改变或修正父类对抽象化的定义
     - 实现化角色：
     - 具体实现化角色：
   + 使用
     - 通过操作抽象化角色来驱动实现化角色执行，相当于委派
     - 抽象化角色接口可以宽于实现化角色
   + 例子
     - jdbc api 与 不同数据库驱动实现，他们都可以独立变化
     - awt: component 与 componentPeer
   + 相似
     - 对象适配器
     - 装饰
     - 策略
 ...
 * 访问者
   + 双重分派
   + 驱动方向：
     - 具体节点接受访问者，访问者再访问具体节点
   + 角色
     - 抽象访问者：声明了一个或多个访问操作
     - 具体访问者：
     - 抽象节点：声明一个接受操作，接受一个访问者对象作为一个参量
     - 具体节点：
     - 结构对象：（相当于数据结构）可以遍历结构中所有元素；提供一个高层次接口让访问者对象可以访问每一个元素；
   + 使用
     - 如果结构对象元素经常被修改，则访问者也需要增加方法，所以不适合经常修改的数据结构
     - 
 * 解释器
   + 抽象表达式角色：
   + 终结符表达式角色：
 ...
 * 调停者
   + 抽象调停者
   + 具体调停者：知道所有具体同事类，从具体同事对象接收消息，向具体同事对象发出命令
   + 抽象同事类角色：定义出调停者到同事对象的接口，同事对象只知道调停者不知道其它同事
   + 具体同事类角色：
