...
#### Promise
 * Promise是一个可写的Future, Future自身并没有写操作相关接口，Netty通过Promise用于设置I/O操作结果
 
   
   
### Dubbo架构
 * provider
   + 服务提供者，每启动一个provider则向register注册即发布
 * consumer
   + 服务调用者，每个consumer都向register订阅provider, 获取provider列表后，通过软负载向一个provider调用服务
 * register
   + zookpeer实现，使用发布订阅模式
 * monitor
   + 统计服务的调用次数和调用时间的监控中心
   + 分别监控，consumer, provider
   
#### Dubbo RPC
 * Dubbo RPC远程服务调用默认使用Netty+Dubbo协议实现。
 * 传统RPC服务调用的问题如下：
   + 网络传输方式使用同步阻塞i/o(bio)
   + 序列化方式使用java或json，序列化后的码流太大，浪费带宽
 ...
 * Dubbo RCP框架默认推荐使用Dubbo协议进行通信和数据传输，相比旧的Hessian协议其性能更高，而且支持异步I/O通信。
 * Netty作为Dubbo RPC框架的高性能异步NIO框架，主要职责
   + 提供异步，高性能的NIO通信框架
   + NIO客户端与服务端
   + 心跳检测能力
   + 断连重连机制
