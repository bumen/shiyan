...
public class AStar {
	...
	public boolean findPath(Grid grid) {
		try {
			...
			this.open = new HeapSupport<>(comparator);
...
// 这里不考虑格子被别的坦克占据的情况，AI行走时会判断前方格子是否被占据		
//		return nearByNode1.isWalkable() && !nearByNode1.isTankOccupy()
//				&& nearByNode2.isWalkable()
...
//			stack.pop();
			stack.remove(0); //Stack 类表示后进先出，最后那点为起始点。应该把先进入的删除掉
			endIndex--;
		}
	}
	...
	private void buildPath() {
		...
		if (retractable) {
			...
			int len = path.size();
			for (int i = 0; i < len; i++) {
				if (path.get(i).isWalkable() == false || path.get(i).isTankOccupy()) {
	...
	private boolean isOpen(Node node) {
		return open.toList().contains(node);
	}
